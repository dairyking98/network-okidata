...
DEFAULT_CONFIG = {
    ...
    # Line spacing: 1/6 inch per line
}
...
def embellish_portion(text, is_double, allocated_columns):
    """
    Returns a bytes string that prints exactly allocated_columns columns.
    If is_double is True, only up to floor(allocated_columns/2) characters are taken,
    and they are wrapped with double width on/off commands. The padding added afterward
    is in normal width.
    """
    text = text.strip()
    if is_double:
        ...
        effective = len(truncated) * 2
        result = IBM_COMMANDS["Double Width On"] + truncated.encode("utf-8") + IBM_COMMANDS["Double Width Off"]
    else:
        max_chars = allocated_columns
        truncated = text[:max_chars]
        effective = len(truncated)
        result = truncated.encode("utf-8")
    pad_length = allocated_columns - effective
    result += b" " * pad_length
    return result
...
def embellish_header_line(header, total_width, day_double, date_double,
    ...
    """
    Splits header (e.g. "Sunday 03/23") into day and date parts.
    Allocates roughly half the width to each (with one column for a space),
    and applies double width only to the portions flagged as such.
    Then wraps each portion with non-double modifiers.
    Returns a bytes string that will print exactly total_width columns.
    """
    parts = header.split(maxsplit=1)
    if len(parts) == 2:
        day_text, date_text = parts
    else:
        # For custom header, treat entire header as day_text.
        return embellish_portion(header, day_double, total_width)
    allocated_day = total_width // 2
    allocated_date = total_width - allocated_day - 1
    # Get each portion with its double width applied appropriately.
    day_portion = embellish_portion(day_text, day_double, allocated_day)
    date_portion = embellish_portion(date_text, date_double, allocated_date)
    # Now get the non-double modifiers.
    ...
    final_day = d_prefix + day_portion + d_suffix
    final_date = dt_prefix + date_portion + dt_suffix
...
class CalendarPrinter:
    def __init__(self, master):
        ...
        # Date and Printer Info.
        tk.Label(input_frame, text="Year:").grid(row=0, column=0, sticky="e")
        tk.Entry(input_frame, textvariable=self.year_var, width=6).grid(row=0, column=1)
        tk.Label(input_frame, text="Month:").grid(row=0, column=2, sticky="e")
        tk.Entry(input_frame, textvariable=self.month_var, width=4).grid(row=0, column=3)
        tk.Label(input_frame, text="Day:").grid(row=0, column=4, sticky="e")
        tk.Entry(input_frame, textvariable=self.day_var, width=4).grid(row=0, column=5)
        
        tk.Label(input_frame, text="Printer IP:").grid(row=1, column=0, sticky="e")
        tk.Entry(input_frame, textvariable=self.printer_ip, width=15).grid(row=1, column=1)
        tk.Label(input_frame, text="Port:").grid(row=1, column=2, sticky="e")
        tk.Entry(input_frame, textvariable=self.printer_port, width=6).grid(row=1, column=3)
        
        # Calendar Type.
        cal_type_frame = tk.LabelFrame(input_frame, text="Calendar Type")
        cal_type_frame.grid(row=2, column=0, columnspan=6, pady=5)
        ...
        header_emb_frame.grid(row=3, column=0, columnspan=6, pady=5)
        ...
        border_frame.grid(row=6, column=0, columnspan=6, pady=5)
        ...
        eighth_frame.grid(row=7, column=0, columnspan=6, pady=5)
        ...
        day_emb_frame.grid(row=8, column=0, columnspan=6, pady=5)
        ...
        date_emb_frame.grid(row=9, column=0, columnspan=6, pady=5)
        ...
        btn_frame.grid(row=10, column=0, columnspan=6, pady=5)
    ...
    def generate_calendar(self):
        ...
        if cal_type == "Month":
            ...
            try:
                ...
                return
            output += "Day Calendar: " + d.strftime("%A, %Y-%m-%d") + "\n"
            ...
            week_header = f"Week of {monday.strftime('%Y-%m-%d')} (Week {week_number}) - {monday.strftime('%B %Y')}"
    ...
    def print_calendar(self):
        ...
        if cal_type in ["Day", "Month"]:
            ...
            # Use our new helper to build each header line.
